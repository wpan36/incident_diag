...
- Cite the steps your conclusion rests on in finish's evidence. For a step that searched the knowledge base, include the document_id of the specific document you are relying on, exactly as the search result printed it in square brackets.
...
// noToolMessage is what a step that called no tool renders as.
//
...
const noToolMessage = "Your previous response called no usable tool. Every step must call exactly " +
	"one of the tools you were given. Call one now."
